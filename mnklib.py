...
class MNKLib:
    ...
    def __init__(self, lib, path):
        ...
        mnk.new_mcts.argtypes = [ctypes.c_int, ndpointer(
            ctypes.c_int, flags="C_CONTIGUOUS"), ndpointer(ctypes.c_float, flags="C_CONTIGUOUS")]
        mnk.new_mcts.restype = ctypes.c_void_p

        mnk.destroy_mcts.argtypes = [ctypes.c_int, ctypes.c_void_p]
        mnk.destroy_mcts.restype = None

        mnk.state_apply.argtypes = [ctypes.c_int,
        ...
        mnk.state_get_board.argtypes = [
            ctypes.c_int, ctypes.c_void_p, ndpointer(ctypes.c_int, flags="C_CONTIGUOUS")]
        mnk.state_get_board.restype = None

        mnk.state_get_boards.argtypes = [
            ctypes.c_int, ctypes.c_void_p, ndpointer(ctypes.c_int, flags="C_CONTIGUOUS")]
        mnk.state_get_boards.restype = None

        self.EvalFunction = ctypes.CFUNCTYPE(ctypes.c_void_p)

        mnk.mcts_get_moves.argtypes = [ctypes.c_int, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_double, ctypes.c_int, ndpointer(
            ctypes.c_double, flags="C_CONTIGUOUS"), self.EvalFunction]
    ...
    def get(cls):
        if cls.instance == None:
            cls.instance = cls("libmnk.so", os.path.join(
...
class MCTS:
    def __init__(self, n):
        ...
        self.handle = MNKLib.get().mnklib.new_mcts(
            n, self.boards_buffer, self.probs_buffer)
