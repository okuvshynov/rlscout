...
class AlphaBetaRuntime {
    ...
    struct MaxRunner {
        score_t run(AlphaBeta<State, score_t> &ab, State s, double alpha, double beta) {
    ...
    score_t dispatch_impl(uint32_t x, ArgsT... args, std::integer_sequence<uint32_t, X...>) {
        score_t res;
    ...
    score_t dispatch(uint32_t x, ArgsT... args) {
        if (x >= 0 && x < B) {
            return dispatch_impl<D, ArgsT...>(x, args..., std::make_integer_sequence<uint32_t, B>{});
        }
        return 0;
    }

    public:
    std::vector<std::pair<uint64_t, score_t>> get_move_scores(State state, score_t alpha, score_t beta) {
