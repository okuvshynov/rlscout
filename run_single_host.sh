...
rm -f ./db/othello6x6_models.db
rm -f ./db/othello6x6_samples.db
...
python rlscout/serve_models.py --db=./db/othello6x6_models.db &
python rlscout/serve_samples.py --db=./db/othello6x6_samples.db &

echo "Starting self-play in the background"
python rlscout/selfplay_loop.py --batch_size=64 --games=16384 -t 4 --rollouts=1500 --random_rollouts=50  2> logs/stderr.log &
echo "Starting model training in the background"
python rlscout/train_loop.py --epoch_samples_min=65536 --minibatch_size=64 --minibatch_per_epoch=5000 2> logs/stderr.log & 
